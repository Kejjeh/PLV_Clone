...
3. **streamer-precision-board** — the precision FA-SP streamer board for today
4. **stream-the-stack** — boom_stack-tier-filtered FA SP streamer shortlist

For any specific player flagged in steps 2-4, optionally run
`/triangulate <name>` for the full 3-lens verdict before deciding.

The bundle exists because these four are always run together on a game-day
...
fa_all = get_free_agents(size=2000)             # steps 3-4 pool (never <2000)
...
## Step 4 — stream-the-stack (condensed)

Filter the precision board to boom_stack tier ≥2/4 confirmed probables in the
next 3 days, Connelly-Early-verified against `fa_all`. See `/stream-the-stack`.
...
- Running the streamer boards before roster-verify — need my_tag first
