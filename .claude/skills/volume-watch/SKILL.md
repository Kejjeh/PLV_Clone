---
name: volume-watch
description: Surface PLAYING-TIME movers — players whose validated model volume projection (RoS PA/team-game for hitters, GS/team-game for SPs) diverges most from their naive season pace, ranked by FP IMPACT (volume gap x rh3/rp3 rate), with a live ESPN ownership overlay (MINE / opponent / FA). RISERS = role expanding (lineup-spot promotion, IL-return ramp, rotation entry) BEFORE the counting stats make it obvious; FADERS = role eroding. The volume parallel of /trending — trending = getting physically BETTER, volume-watch = playing MORE. Use when the user asks "who's gaining playing time", "role risers", "volume movers", "whose role is shrinking", "playing time watch", "who's playing more/less than their season line suggests", "lineup promotions", "rotation entries", "volume watch". FADER flags carry a kind since 2026-08-29: **FADER (ROLE)** = genuine lineup erosion, act on it; **FADER (AVAILABILITY)** = the when-active role is intact and the gap prices expected missed time (age/injury history) — an injury-risk statement, NEVER a daily-sit signal (use the card's in-role volume for start/sit). On the SP side the role mechanism is the ROTATION TURN, measured in team games: a stretched turn (>5.5 — six-man, piggyback, innings limit) is ROLE, a full 5.0 turn below pace is AVAILABILITY, and the card prints "every N team games (X starts/wk when active)" — use THAT, not the 1.19 league default, in SP-cap and playoff start-count math (Glasnow canonical: LAD six-man, 1.02/wk). One owner: lib/volume_semantics.py (Muncy/Peters/Glasnow canonicals).

DISPLAY/DECISION layer only — never moves an rh3/rp3 projection.
---
