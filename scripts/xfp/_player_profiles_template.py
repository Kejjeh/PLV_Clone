...
.tri-xlink { font-size: .5em; font-weight: 500; margin-left: .9em; color: var(--dim);
             text-decoration: none; border: 1px solid var(--border); border-radius: 3px;
             padding: .25em .6em; vertical-align: middle; }
.tri-xlink:hover { color: var(--accent); border-color: var(--accent); }
...
/* 2026-07-18: fixed layout crushed columns until headers were unreadable.
   Columns now size to their content (headers set the floor) and the wrapper
   scrolls horizontally when the table outgrows the viewport. Tradeoff: a
   filter popover near the right edge can clip against the scroll wrapper. */
.table-scroll:has(table.alltable) { overflow-x: auto; }
table.alltable { width: max-content; min-width: 100%; table-layout: auto;
    margin-bottom: 0;
...
function openModal(role, id) {
  ...
  hero += `<div><div class="hero-name">${last.player_name}${partialBadge(last)}<a class="tri-xlink" href="triangulate.html#p=${encodeURIComponent(last.player_name)}" title="Three-lens verdict card">verdict →</a></div>`;
...
const COL_TOOLTIPS = {
  ...
  STUFF: 'Pitcher stuff domain 20-80 — within-year z-score. SP: SwM+Velo. RP: 0.85*SwM + 0.15*Velo. Higher = better. THE load-bearing SP pillar (fwd r .57 — out-predicts FP itself; ~76% FP-faithful weight, SWING_MISS dominant). For RPs its forward value is JOB SECURITY (closer retention 33/52/88% by tercile), not run prevention.',
  MOVEMENT: 'Pitcher movement domain 20-80 (SP). RP rows show BATTED_BALL here (GB + Bulk IP). Higher = better. FP-faithful weight ~14% (shipped OVERALL uses 35% — 3x its forward value); display trait.',
  CONTROL: 'Pitcher control domain 20-80 — within-year z-score. SP: WalkAvoid+CalledStrike. RP: 0.85*WalkAvoid + 0.15*CalledStrike. Higher = better. SP FP-faithful weight ~10%; RP forward FP ~ZERO (2026-07-04) — RP CONTROL is a scouting descriptor, excluded from FPwt.',
  ...
  CONTACT_QUALITY: 'xwOBA-on-contact 20-80 — quality of contact, not quantity. Higher = better. Forward-FP weight ~0 (2026-07-04) — use as the xwOBACON DIAGNOSTIC (slump/luck read), not a forward signal.',
  SPRAY_PROFILE: 'Pull-side fly ball rate 20-80. Higher = more pulled-FB damage. Forward-FP weight ~0 (2026-07-04) — style descriptor.',
