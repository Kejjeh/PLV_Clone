"""Shared helper for rh3 INTERACTION-term candidate validation.

Wraps _validate_rh3_v3_helper to (1) load and prep the rolling frame
with all RH3_FEATS computed (including shrinkage), (2) add a single
product-of-two-existing-columns interaction column, then (3) run the
same cross-year LOO eval used elsewhere.

Pre-registered: see data/research/validation_runs/*_2026-05-24.md.
"""
...
def run_interaction_eval(
    *,
    ...
    """Eval an interaction column `name` = col_a * col_b vs full RH3_FEATS.

    Both col_a and col_b must exist post-load_and_prep_rh3_inputs().
    """
    rolling = load_and_prep_rh3_inputs()

    for c in (col_a, col_b):
        assert c in rolling.columns, (
            f"interaction input column '{c}' missing from prepped DataFrame"
        )

    rolling[name] = rolling[col_a].astype(float) * rolling[col_b].astype(float)
    # Conservative NaN handling — fill with column mean over training years
    if rolling[name].isna().any():
        mu = rolling.loc[rolling["year"].isin(rh3.TRAIN_YEARS), name].mean(
            skipna=True
        )
        rolling[name] = rolling[name].fillna(mu)

    n_missing = rolling[name].isna().sum()
    print(f"  interaction '{name}' = {col_a} * {col_b}: NaN after fill = {n_missing}")
    print(
        f"  range: [{rolling[name].min():.4f}, {rolling[name].max():.4f}]  "
        f"mean={rolling[name].mean():.4f}"
    )
    ...
    # Convergence per split_day (Rule 8)
