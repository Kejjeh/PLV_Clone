...
def _select_rp3_path():
    """Pick between il_fixed shim and canonical rp3 projections with a freshness check.

    The shim (xfp_rp3_projections_il_fixed.csv) is regenerated by
    fix_il_flag_from_espn.py and overrides is_on_il_at_split with live ESPN
    status. Without this step, a fresh rp3 pipeline run leaves the shim stale,
    shadowing newer canonical projections (the bug pattern documented in
    feedback_il_fixed_shim_resolution.md).

    Rule:
      - if shim is missing → use canonical (no warning).
      - if shim mtime is within 24h of canonical mtime → use shim (preferred).
      - if shim is >24h older than canonical → print a clear warning and fall
        back to canonical. We do NOT raise; the dashboard must keep building.
        The refresh_dashboards.py pipeline owns the freshness contract via
        step 2a, and ESPN outages must not break the build.
    """
    canonical = OUT / 'xfp_rp3_projections.csv'
    shim = OUT / 'xfp_rp3_projections_il_fixed.csv'
    if not shim.exists():
        return canonical
    try:
        shim_mtime = shim.stat().st_mtime
        canon_mtime = canonical.stat().st_mtime
        # If canonical is newer than shim by more than 24 hours, shim is stale.
        stale_seconds = canon_mtime - shim_mtime
        if stale_seconds > 24 * 3600:
            hours = stale_seconds / 3600
            print(f'  ⚠ xfp_rp3_projections_il_fixed.csv is {hours:.1f}h older '
                  f'than xfp_rp3_projections.csv — falling back to canonical. '
                  f'Re-run scripts/xfp/fix_il_flag_from_espn.py to refresh the shim.')
            return canonical
    except OSError:
        return canonical
    return shim
