...
# NOTE (sb_target_fix 2026-07-10): stolen_base_* NEVER appears in the statcast
# `events` column — SBs are baserunning events, not batter-PA outcomes — so
# is_sb / sb / sb_per_pa derived from these are identically 0. They are kept
# only so the schema stays stable. SB points enter the TARGET via the MLB-API
# season-rate allocation (see rrbisb_rates below), same as R/RBI. A true
...
def aggregate_window(pitches: pd.DataFrame) -> pd.DataFrame:
    ...
    # FP target on the window. NOTE: no SB term — statcast-events sb is
    # identically 0 (see SB_EVENTS note above); SB joins R/RBI at outer scope
    # via the MLB-API season-rate allocation (rrbisb_rates).
    agg['fp_total'] = (
        agg['tb'] + agg['bb'] + agg['hbp'] - agg['k']
    ).astype(float)  # core_fp; R/RBI/SB added at outer scope from MLB API
    agg['core_fp_per_pa'] = agg['fp_total'] / pa
    return agg


def fp_per_pa_with_rrbisb(window_agg: pd.DataFrame, rrbisb_rates: pd.Series) -> pd.Series:
    """Add R + RBI + SB per PA (proportional from MLB API season totals).
    rrbisb_rates is keyed on batter, value is total (r+rbi+sb)/season-pa.
    """
    pa = window_agg['pa'].replace(0, np.nan)
    rrbisb_per_pa = window_agg['batter'].map(rrbisb_rates).fillna(0.0)
...
def build_year(year: int, season_start: pd.Timestamp) -> pd.DataFrame:
    ...
    # MLB API per-batter R/RBI/SB rates from existing counting-stats cache.
    # SB added 2026-07-10 (sb_target_fix): the statcast-events path never sees
    # SBs, so the full-FP target must pick them up here exactly like R/RBI —
    # season totals allocated proportionally per PA. This also resolves the
    # prior/target unit inconsistency (the Marcel prior from
    # build_hitters_multiyr.py has included SB since 2026-05-06).
    rrbisb_rates: pd.Series
    ...
    if counts_path.exists():
        ...
        if 'mlb_pa' in cnts.columns:
            cnts['rrbisb_per_pa'] = (
                cnts['mlb_r'].fillna(0) + cnts['mlb_rbi'].fillna(0) + cnts['mlb_sb'].fillna(0)
            ) / cnts['mlb_pa'].replace(0, np.nan)
            rrbisb_rates = cnts.set_index('batter')['rrbisb_per_pa']
        else:
    ...
    for split_day in splits_to_use:
        ...
        if in_progress_row:
            ...
            # Full target = after-window core (TB+BB+HBP-K per PA) + season-rate
            # R/RBI/SB allocation. Season rates are legitimately "future" here —
            # the target IS the future outcome (same pattern R/RBI always used).
            merged['ros_full_fp_per_pa'] = (
                merged['ros_core_fp_per_pa'] + merged['batter'].map(rrbisb_rates).fillna(0.0)
            ).round(4)
