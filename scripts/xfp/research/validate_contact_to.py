"""Validation script for `contact_to` as a rh3 v3 candidate feature.

Pre-registered: data/research/validation_runs/contact_to_2026-05-24.md.

Hypothesis: Cumulative contact-event count (season-to-date) adds
independent predictive lift on RoS FP/PA over the full RH3_FEATS baseline.

Run with:
    PYTHONIOENCODING=utf-8 python scripts/xfp/validate_contact_to.py
"""
...
def main() -> None:
    print("=== /validate-feature: contact_to (rh3 v3 candidate, ceiling audit 2026-05-24) ===")
    print("Pre-reg: data/research/validation_runs/contact_to_2026-05-24.md")
    print()
    result = run_candidate_eval(
        candidate="contact_to",
        expected_sign="+",
        pre_reg_path=next(
