...
def _repo_root() -> Path:
    ...
    Was `Path(__file__).resolve().parents[2]`, correct while this file lived at
    scripts/xfp/. It was archived into scripts/xfp/research/ on 2026-07-19
    (commit b42b561) and the hardcoded depth silently began resolving to
    `<repo>/scripts`, so every optional input below .exists()==False and got
    filled with 0.0 — a Rule 9 baseline degraded to three constant features.
    Marker-based so a future move cannot reintroduce it. (Fixed 2026-07-28.)
    """
    here = Path(__file__).resolve()
    for cand in here.parents:
        if (cand / 'pyproject.toml').is_file():
            return cand
    raise SystemExit(f'ABORT — cannot locate repo root (no pyproject.toml above {here})')
...
BX_PRIORS    = ROOT / 'data' / 'research' / 'xfp_cache' / 'bx_priors_2018_2026.csv'

# Rule 9: a baseline input that is missing must ABORT, never silently become a
# constant. Optional-with-0.0-fallback is how the degradation above went unnoticed.
for _name, _p in [('ROLLING_CSV', ROLLING_CSV), ('MULTIYR_CSV', MULTIYR_CSV),
                  ('H2_LOCKED', H2_LOCKED), ('XWOBA_RESID', XWOBA_RESID),
                  ('ROS_OPP_SP', ROS_OPP_SP), ('BX_PRIORS', BX_PRIORS)]:
    if not _p.is_file():
        raise SystemExit(
            f'ABORT — Rule 9 baseline input missing: {_name} = {_p}\n'
            f'  The baseline must contain every production feature. Refusing to '
            f'run with a degraded baseline.')
...
# Rule 9: the baseline IS the live production feature list — never a copy.
# The previous hardcoded list went stale at 21 features when `bx_prior_h` was
# promoted 2026-07-10 (production had 22), which would understate the baseline
# and overstate any candidate's lift. Importing it makes drift impossible.
if str(ROOT / 'src') not in sys.path:
    sys.path.insert(0, str(ROOT / 'src'))
from plv_clone.models.xfp.rh3 import RH3_FEATS  # noqa: E402
...
def main():
    ...
    # bx_prior_h — box-score-era ensemble prior, promoted into RH3_FEATS
    # 2026-07-10 (after this harness was written). Mirrors rh3.main().
    bx = pd.read_csv(BX_PRIORS)[['mlbam', 'year', 'bx_prior_h']].rename(
        columns={'mlbam': 'batter'})
    rolling = rolling.merge(bx, on=['batter', 'year'], how='left')
    _ym = rolling.groupby('year')['bx_prior_h'].transform('mean')
    rolling['bx_prior_h'] = (rolling['bx_prior_h'].fillna(_ym)
    ...
    # Rule 9: silently dropping an absent production feature shrinks the baseline
    # and inflates every candidate's measured lift. Abort instead of warning.
    missing_base = [f for f in RH3_FEATS if f not in rolling.columns]
    if missing_base:
        raise SystemExit(
            f'ABORT — Rule 9 violation: baseline is missing production features '
            f'{missing_base}.\n  Wire them into the assembly above; do not run '
            f'against a reduced baseline.')
    base_feats_use = RH3_FEATS
    print(f'Baseline: {len(base_feats_use)}/{len(RH3_FEATS)} production features (complete)')
