...
def main():
    ...
    fa_sp = get_free_agents(position='SP', size=200)
    ...
    fa_sp = fa_sp.dropna(subset=['proj']).sort_values('proj', ascending=False).head(5)

    fa_all = get_free_agents(size=300)
    ...
    fa_rp = get_free_agents(position='RP', size=100)
