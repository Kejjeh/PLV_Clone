"""Validation script for `pa_per_started_game_to` as a rh3 v3 candidate feature.

Pre-registered: data/research/validation_runs/pa_per_started_game_to_2026-05-23.md.

Hypothesis: PA-per-started-game (in-season-to-date) adds independent
predictive lift on RoS FP/PA over the full RH3_FEATS baseline.

Run with:
    PYTHONIOENCODING=utf-8 python scripts/xfp/validate_pa_per_started_game_to.py
"""
...
def main() -> None:
    print("=== /validate-feature: pa_per_started_game_to (rh3 v3 candidate) ===")
    print(
        "Pre-reg: data/research/validation_runs/pa_per_started_game_to_2026-05-23.md"
    )
    print()
    result = run_candidate_eval(
        candidate="pa_per_started_game_to",
        expected_sign="+",
        pre_reg_path=Path(__file__).resolve().parents[2]
        / "data"
        / "research"
        / "validation_runs"
        / "pa_per_started_game_to_2026-05-23.md",
    )
