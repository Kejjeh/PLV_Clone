"""Validation script for `started_pct_to` as a rh3 v3 candidate feature.

Pre-registered: data/research/validation_runs/started_pct_to_2026-05-23.md.

Hypothesis: Fraction of team games started (in-season-to-date) adds
independent predictive lift on RoS FP/PA over the full RH3_FEATS baseline.

Run with:
    PYTHONIOENCODING=utf-8 python scripts/xfp/validate_started_pct_to.py
"""
...
def main() -> None:
    print("=== /validate-feature: started_pct_to (rh3 v3 candidate) ===")
    print("Pre-reg: data/research/validation_runs/started_pct_to_2026-05-23.md")
    print()
    result = run_candidate_eval(
        candidate="started_pct_to",
        expected_sign="+",
        pre_reg_path=Path(__file__).resolve().parents[2]
        / "data"
        / "research"
        / "validation_runs"
        / "started_pct_to_2026-05-23.md",
    )
